...
def compare_pose_bdp(user, ref):
    ...
    weights = {k: 0.2 for k in body_vectors}
    weights['direction'] = 0.6
...
def compare_pose_bdp_double_frame(user_now, user_prev, ref_now, ref_prev, alpha=0.3):
    ...
    final_score = (1 - alpha) * base_score + alpha * motion_score

    logging.debug(f"[DoubleFrame] base={base_score:.2f}, motion={motion_score:.2f}, final={final_score:.2f}")
